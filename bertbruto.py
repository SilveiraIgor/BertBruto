...
def selecionar_aleatoriamente(dataframe_treino, instancia):
  ...
  for idx,linha1 in aleatorio.iterrows():
    nova_linha = pd.DataFrame([{"essay1_text": linha1['essay_text'], "essay2_text": instancia['essay_text'], 'label': (linha1['grades'][0] - instancia['grades'][0])//40 }])
    df = pd.concat([df, nova_linha], ignore_index=True)
  return df
...
def arredondar_notas(lista_notas):
  return [round(nota) for nota in lista_notas]
...
def test_model(dataframe_train, dataframe_test, model, tokenizer):
      ...
      with torch.no_grad():
        for i in range(0, len(dataframe_test)):
            ...
            arredondadas = arredondar_notas(outputs.flatten().tolist())
            y_pred.append(mais_frequente(arredondadas))
            #loss = criterion(outputs, labels
